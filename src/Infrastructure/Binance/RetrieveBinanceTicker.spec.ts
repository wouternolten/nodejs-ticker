...
describe('retrieveTicker test', () => {
    ...
    it('Should throw an error when an invalid charachter is used', async () => {
        ...
        try {
            await retrieveBinanceTicker.retrieveTicker('INVALIDSYMBOL14!@#');
        } catch (e) {
            expect(e).toBeInstanceOf(IllegalCharacterException);
        }
    });
    ...
    it('Should throw an error when an url is not found', async () => {
        ...
        try {
            await retrieveBinanceTicker.retrieveTicker('');
        } catch (e) {
            expect(e).toBeInstanceOf(UrlNotFoundException);
        }
    });
    ...
    it('Should give back a valid ticker when url is successfully called and valid symbol is used', async (done) => {
        ...
        retrieveBinanceTicker
            .retrieveTicker('BTCUSDT')
            .then((ticker: Ticker) => {
